Tridom: A Minimal Causal Unit for
Substrate-Invariant Network Dynamics
...
Triadic Threshold Theorem). We further show that the binary sign pattern (topology
+ E/I labelling) is substrate-invariant: it is preserved across biological, digital, and
...
chaotic); (iii) a PyTorch reference implementation with local Hebbian plasticity,
calibrated on delay and hysteresis tasks; and (iv) a self-organising multi-agent
system (TridomGroupSystem v2 / Nona2) with fully endogenous birth, death, and
replication. Tridom's contribution is not incremental: it shifts the ontological grain
from the motif as a network statistic to the motif as a formal, persistent,
substrate-invariant causal unit.
...
support  the  Triadic  Threshold  Theorem  showing  that  n  =  3  is  the  minimal  size  for
...
replication) governed entirely by a vitality metric, without any external training oracle.
...
4. The Triadic Threshold Theorem

Theorem (Triadic Threshold). For any strongly connected signed directed graph G on n nodes:
...
Corollary.  n  =  3  is  the  smallest  integer  at  which  substrate-invariant  non-trivial  causal
...
Proof sketch. The n = 1 and n = 2 cases follow directly from the Thomas–Kaufman conditions2:
...
random initial conditions, establishes the lower bound. The formal proof of this theorem is an
open contribution item (see Section 8).

5. Atlas of 78 Canonical Topologies
...
×1.64. The triadic loop develops a spontaneous internal directional representation via local Hebbian
plasticity, without any external direction signal.
...
TGS  v2  is  a  self-organising  population  of  Tridom  groups  implementing  a  fully  endogenous
lifecycle. No external training signal, reward function, or evolutionary operator is required.
...
of the Triadic Threshold Theorem rather than independent results.
...
We have introduced Tridom as a formal, substrate-invariant causal unit defined by a triple (G,
σ,  [f]):  signed  strongly  connected  graph  on  3  nodes,  binary  excitatory  /  inhibitory  edge
labelling, and dynamical equivalence class. The Triadic Threshold Theorem establishes that n
